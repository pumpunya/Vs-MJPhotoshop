<?php
        $xid = $booking->bookingID;
        $editimagedeposit = DB::connection('mysql')
        ->select('SELECT * FROM editdeposit
        WHERE bookingID=?', [$xid]);
                            ...
                            <table class="table">
                                <tbody>
                                    ...
                                    <tr><th> สถานะการชำระเงิน </th><td><?php
                                        ...
                                        ?> </td></tr>
                                        <tr><th> ค่ามัดจำ </th><td> {{ $booking->price }} </td></tr>
                                        <tr><th> ใบเสร็จค่ามัดจำ </th><td> <img src="{{ url('/') }}/images/{{ $booking->imagedeposit }}" width= '300px' height='300px'></td></tr>
                                        <tr><th> ยอดค้างชำระ </th><td> {{ $booking->comment }} </td></tr>

                                        @foreach($editimagedeposit as $img)
                                        <tr><th> ใบเสร็จค้างชำระ</th><td> <img src="{{ url('/') }}/images/{{ $img->imagedeposit2 }}" width= '300px' height='300px'></td></tr>
                                        @endforeach
                                </tbody>
                            </table>
                        </div>
