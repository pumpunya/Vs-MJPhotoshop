<?php
    $xid = $printphotodetail->printphotoID;

        $editimage = DB::connection('mysql')
        ->select('SELECT * FROM editpayment
        WHERE printphotoID=?', [$xid]);
                        ...
                        <div class="table-responsive">
                            <table class="table">
                                <tbody>
                                    ...
                                    @foreach($editimage as $img)
                                    <tr><th> ใบเสร็จค้างชำระ</th><td> <img src="{{ url('/') }}/images/{{ $img->imagebill2 }}" width= '300px' height='300px'></td></tr>
                                    @endforeach
                                </tbody>
                            </table>
                        </div>
